...
/** "或许你想看?" 推荐影片列表（最多 4 张） */
const recommendList = ref<FilmListItem[]>([])
...
async function loadRecommend(): Promise<void> {
  ...
  try {
    ...
    const pool: FilmListItem[] = []
    const seen = new Set<string>()
    const push = (items?: FilmListItem[]): void => {
      if (!items) return
      for (const it of items) {
        const key = String(it.id ?? it.mid ?? it.name)
      ...
      >
        ...
        <ul class="nf-recommend__grid">
          <li
            ...
            :key="String(item.id ?? item.mid ?? item.name)"
