# GoFilm 设计 Token 规范（暗色主题）

> 适用于 `client-v2/` 用户端 + 管理端，Netflix / Disney+ 视觉基调，紫品红渐变继承自旧站作为强调色。所有 Token 以 CSS 自定义属性输出，前缀统一 `--gf-`。
...
- 辅助强调：**品紫渐变 `#9b49e7 → #4ad1e5`**（继承旧站，用于 Logo / 进度条 / 后台高亮 / 登录 CTA）
